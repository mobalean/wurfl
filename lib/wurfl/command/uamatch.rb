...
=begin
  ...
  The implementation of the Levenshtein distance used here is based on the algorithm 
  found in the Text gem originally written by Paul Battley (pbattley@gmail.com)
  ...
  length of the user agent this brought an over all increase by a factor of about 40, 
  although it renders the code slightly unreadable.  In general the longer the 
  user agent string and the greater the result distance, the longer the
  computation takes.  
  
  The uamatch command itself is based on the inspector command found in this library.
  
  uamatch author:  Kai W. Zimmermann (kwz@kai-zimmermann.de)
=end
  class WurfUamatch
  ...
  def execute
    ...
    begin
      ...
      uamatch = WurfUamatch.new(handsets)
    rescue => err
